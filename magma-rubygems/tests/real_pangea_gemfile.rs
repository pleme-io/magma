...
fn parses_real_pangea_gemfile() {
    ...
    assert!(
        matches!(&m.sources[0], Source::RubyGemsOrg { mirror_url: Some(u) } if u == "https://rubygems.org")
    );
    ...
    assert!(
        m.deps.len() >= 15,
        "expected ≥15 deps, got {}",
        m.deps.len()
    );

    // Every pangea-* dep is path-sourced.
    let pangea_path_count = m
        .deps
        .iter()
        .filter(|d| d.name.starts_with("pangea-") && matches!(&d.source, Some(Source::Path { .. })))
        .count();
    assert!(
        pangea_path_count >= 14,
        "expected ≥14 path-sourced pangea-* deps, got {pangea_path_count}"
    );
    ...
    assert!(
        rspec.source.is_none(),
        "rspec should default to the top-level rubygems source"
    );
}
