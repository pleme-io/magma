...
pub async fn run(plan: MigrationPlan) -> Result<MigrationReceipt, MigrationError> {
    ...
    if !plan.dry_run {
        write_state(&plan.to.state_path, &to_state).await
            .map_err(|e| MigrationError::Stalled {
                phase:      "write_to".into(),
                reason:     e.to_string(),
                checkpoint: plan.to.state_path.with_extension("tfstate.tmp"),
            })?;
        write_state(&plan.from.state_path, &from_state).await
            .map_err(|e| MigrationError::Stalled {
...
fn execute_move(
    ...
    let position = from_state.resources.iter().position(|r| {
        resource_address_string(&r.address) == mv.source_address
    ...
    let (new_type, new_name) = parse_address(&mv.target_address)
        .ok_or_else(|| MigrationError::Validation {
...
mod tests {
    use super::*;
    use magma_types::{
        InstanceStatus, ModulePath, ProviderReference, ResourceKind, ResourceTypeId,
        StateInstance,
    };
    ...
    async fn migrate_one_resource_atomically() {
        ...
        from.resources.push(make_resource("aws_iam_role", "k3s_node"));
        let to = fresh_state();
        ...
        let plan = MigrationPlan {
            from: WorkspaceRef { name: "k3s_permissions".into(), state_path: from_path.clone() },
            to:   WorkspaceRef { name: "platform_iam".into(),    state_path: to_path.clone() },
    ...
    async fn migrate_validation_rejects_missing_source() {
        ...
        let plan = MigrationPlan {
            from: WorkspaceRef { name: "f".into(), state_path: from_path },
            to:   WorkspaceRef { name: "t".into(), state_path: to_path   },
            moves: vec![ResourceMove {
    ...
    async fn migrate_dry_run_leaves_state_files_unchanged() {
        ...
        let plan = MigrationPlan {
            from: WorkspaceRef { name: "f".into(), state_path: from_path.clone() },
            to:   WorkspaceRef { name: "t".into(), state_path: to_path.clone() },
            moves: vec![ResourceMove {
                source_address: "aws_iam_role.k3s_node".into(),
    ...
    fn parse_address_basic() {
        assert_eq!(parse_address("aws_iam_role.k3s_node"), Some(("aws_iam_role", "k3s_node")));
        assert_eq!(parse_address("aws_iam_role.k3s_node[0]"), Some(("aws_iam_role", "k3s_node")));
