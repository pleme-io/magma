...
mod tests {
    ...
    // ── Condition Status K8s wire-format round-trip ──────────────

    /// `.name()` (auto-generated by Discriminant) and `FromStr` (auto-
    /// generated by FromStrKind) together implement the typed K8s
    /// wire-format round-trip.
    #[test]
    fn condition_status_round_trip_through_wire_format() {
        for status in [
            ConditionStatus::True,
            ConditionStatus::False,
            ConditionStatus::Unknown,
        ] {
            let wire = status.name();
            assert_eq!(
                ConditionStatus::from_str(wire).unwrap(),
                status,
    ...
    /// FromStrKind is case-sensitive — K8s "True" wire format does NOT
    /// match "true". This is intentional — the K8s API server rejects
    ...
    /// Unknown wire strings produce a typed parse error listing every
    /// expected variant. The error is operator-actionable.
    #[test]
    fn condition_status_parse_error_lists_expected() {
        let err = ConditionStatus::from_str("Maybe").unwrap_err();
        let msg = format!("{err}");
